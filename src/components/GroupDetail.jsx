...
export default function GroupDetail({ group, allUsers, onUpdate, onBack, currentUser }) {
  ...
  useEffect(() => {
    const recurring = g.recurringExpenses || [];
    const pending = recurring.filter(r => r.active && getRecurringOccurrences(r).length > 0);
    ...
    <div style={{ fontFamily: "var(--font-sans)", maxWidth: 640, margin: "0 auto" }}>
      ...
      <div style={{ padding: "0 1rem 2rem" }}>
        <div style={{ display: "flex", gap: 6, marginBottom: "1.5rem", flexWrap: "wrap" }}>
          ...
          <NavBtn k="dauerauftraege" label="Daueraufträge" />
          ...
          <div style={{ animation: "fadeIn 0.2s ease" }}>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              {(g.recurringExpenses || []).map(r => (
                <div key={r.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "13px 14px", background: "var(--color-background-primary)", boxShadow: "var(--shadow-sm)", borderRadius: "var(--radius)" }}>
